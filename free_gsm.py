...
parser.add_argument("--v_idx",type=int,default=-1)
...
model_map = {
    ...
    "d_qwen-1.5b":"/lzh/model/distill-qwen-1.5b",
}
...
data_map = {
    ...
    "d_qwen-1.5b":"/lzh/cot/distill/GPU_run/gsm/q_gsm_300_judge.jsonl",
}
...
if args.steering:
    ...
    if "qwen" in args.model_name:
        H = np.load("/lzh/cot/distill/eigenvectors_qwen_1.5.npy")
    else:
        H = np.load("/lzh/cot/distill/eigenvectors_llama.npy")
        
    steer_v = torch.from_numpy(H[:,args.v_idx]).to("cuda")
    ...
    def adjust_residual_hook():
        def hook_fn(module, input, output):
            ...
            resid_hat = resid_float + args.strength * steer_v
    ...
    for i,ele in enumerate(processed_questions):
        ...
        ele["free_final_cot"] = response
        ele["free_final_tokens"] = token_num/(i+1)
    with open(f"./{args.model_name}_final_free_gsm_300_judge_{args.v_idx}_{args.strength}.jsonl","w",encoding = "utf8") as fw:
